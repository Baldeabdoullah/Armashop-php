...
if (isset($_POST['update'])) {
    ...
    $update_id = $_POST['update_categorie_id'];
    $update_nom = $_POST['update_nom'];

    $update = mysqli_query($connection, "Update `categorie` set 
    nom  = '$update_nom' where id_categorie='$update_id'");

    if ($update) {

        header('location:../dashboard.php');
    } else {
        $display_message = "Il y'a une erreur dans la modification de la categorie";
    }
}
...
<body>
    ...
    <div class="formular-container">
        <?php
        if (isset($_GET['categorie_edit'])) {
            $id =  $_GET['categorie_edit'];

            $result = mysqli_query($connection, "Select * from `categorie` where id_categorie=$id");

            while ($row = mysqli_fetch_assoc($result)) {

        ?>

                <h3>Modifier la Cathegorie</h3>
                <form method="post" enctype="multipart/form-data">

                    <div>
                        <input type="hidden" value='<?php echo $row['id_categorie'] ?>' name="update_categorie_id">
                    </div>

                    <div>
                        <input type="text" value='<?php echo $row['nom'] ?>' name="update_nom" placeholder="nom">
                    </div>
